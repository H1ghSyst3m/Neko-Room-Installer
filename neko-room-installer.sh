...
echo -e "    - Free UDP port range: 59000-59099."
...
validate_and_get_domain() {
  while true; do
    ...
    if validate_domain "$DOMAIN"; then
      ...
      if echo "$SERVER_IPS" | grep -q "^$DOMAIN_IP$"; then
...
check_ports() {
  ...
  if [[ ! "$start_port" =~ ^[0-9]+$ ]] || [[ ! "$end_port" =~ ^[0-9]+$ ]] || ((start_port > end_port)); then
    echo -e "${RED}Invalid port range: $port_range. Please use the format START-END (e.g., 59000-59099).${NC}"
    return 1
  fi
...
validate_room_port_range() {
  while true; do
    validate_input "Enter the Room port range [Default: 59000-59099]:" ROOM_PORT_RANGE "59000-59099"
...
summary() {
  ...
  echo -e "${YELLOW}Public IP:${NC} $(curl -s ifconfig.me)"
  ...
  while true; do
    ...
    case $proceed in
      [Yy]* ) break ;;
      [Nn]* ) echo -e "${RED}Installation aborted by user.${NC}"; exit 1 ;;
      * ) echo -e "${RED}Please answer yes or no.${NC}" ;;
    esac
  done

  echo -e "\n${AQUA}Proceeding with the installation...${NC}\n"
}
...
services:
  neko-rooms:
    ...
    environment:
      ...
      - "NEKO_ROOMS_NAT1TO1=$(curl -s ifconfig.me)"
